...
export default function SignupPage() {
  ...
  const handleOAuthSignup = async (provider: 'google' | 'github') => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider,
        options: {
          redirectTo: `${window.location.origin}/auth/callback?redirectTo=/dashboard?new_user=true`,
        },
      })
  ...
  return (
    <AuthCard
      ...
      subtitle="Start organizing your product images"
      footer={{
    ...
    >
      ...
      <p className="mt-6 text-xs text-center text-gray-500">
        ...
        <a href="/terms" className="text-treez-purple hover:text-treez-cyan transition-colors">
          Terms of Service
        </a>{' '}
        and{' '}
        <a href="/privacy" className="text-treez-purple hover:text-treez-cyan transition-colors">
          Privacy Policy
        </a>
      </p>
    </AuthCard>
